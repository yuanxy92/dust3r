...
def rgbdpano_to_point_cloud_w_skymask(rgb_image, depth_image, conf_image, im_range, resolution, sky_mask):
    """
    Convert RGB and Depth images into a point cloud
    :param rgb_image: HxWx3 numpy array representing the RGB image
    :param depth_image: HxW numpy array representing the depth image
...
def rgbdpano_to_ptmesh_w_skymask(rgb_image, depth_image, conf_image, im_range, resolution, panocenter, skymask, angle_th=85, conf_th=0):
    """
    Convert RGB and Depth images into a point cloud
    :param rgb_image: HxWx3 numpy array representing the RGB image
    :param depth_image: HxW numpy array representing the depth image
...
def pano_stitch(ba_images, outdir, outname, blender=no_blend, equalize=False, crop=False):
    # compute range
    has_mask = False
    if ba_images[0].skymask is not None:
        has_mask = True
        patches_skymask = []
    ...
    for ba_image in ba_images:
        ...
        if has_mask:
            ba_image.skymask = _add_weights(ba_image.skymask)
    # estimate resolution
    ...
    for reg in ba_images:
        ...
        # warp mask image
        if has_mask:
            warped_mask = cv2.remap(reg.skymask, x_pr[:, :, 0], x_pr[:, :, 1],
                            cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT)
            warped_mask[..., 3] = warped_mask[..., 3] * (~mask)
            patches_skymask.append((warped_mask, mask, irange))
    ...
    if has_mask:
        mosaic_skymask = blender(patches_skymask, shape)
    # segment sky
    # convert RGB-Depth image to point clouds
    if has_mask:
        ptcloud = rgbdpano_to_point_cloud_w_skymask(mosaic_rgb, mosaic_dist[:, :, 0], mosaic_conf[:, :, 0], im_range, resolution, mosaic_skymask[:, :, 0])
        mesh = rgbdpano_to_ptmesh_w_skymask(mosaic_rgb, mosaic_dist[:, :, 0], mosaic_conf[:, :, 0], 
            im_range, resolution, ba_images[0].panocenter, mosaic_skymask[:, :, 0])
    else:
    ...
    mosaic_conf_full = remap_panorama_to_full(mosaic_conf, (im_range[0][0], im_range[1][0]), 
            (im_range[0][1], im_range[1][1]))
    mosaic_skymask_full = remap_panorama_to_full(mosaic_skymask, (im_range[0][0], im_range[1][0]), 
            (im_range[0][1], im_range[1][1]))
    ...
    if has_mask:
        mosaic_skymask_full_norm = mosaic_skymask_full / np.max(mosaic_skymask_full) * 255
        cv2.imwrite(f'{outdir}/{outname}_sky_full.png', mosaic_skymask_full_norm.astype(np.uint8))
    ...
    return mosaic_rgb_full, mosaic_dist_full, mosaic_conf_full, resolution, im_range
